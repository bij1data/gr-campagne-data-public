...
mkdir -p data/stembureaus/
# for each municipality that BIJ1 is running in for GR2022
array=( Amsterdam Rotterdam Utrecht Almere Delft )
for gemeente in "${array[@]}"
do
	...
	# convert to csv
    python -c "import pandas as pd; pd.read_json('${gemeente}.json').to_csv('data/stembureaus/${gemeente}.csv', index=False)"
	rm "${gemeente}.json"
done
